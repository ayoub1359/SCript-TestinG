...
local function CalcSizes(w, h)
    return {
        fontTitle  = math.clamp(math.floor(w*0.087), 11, 26),
        fontNormal = math.clamp(math.floor(w*0.068), 10, 22),
        fontSmall  = math.clamp(math.floor(w*0.054), 9,  18),
        fontTimer  = math.clamp(math.floor(w*0.048), 8,  16),
        titleH     = math.clamp(math.floor(h*0.078), 26, 48),
        btnH       = math.clamp(math.floor(h*0.072), 22, 46),
        inputH     = math.clamp(math.floor(h*0.068), 20, 44),
        timerH     = math.clamp(math.floor(h*0.038), 14, 24),
        rowH       = math.clamp(math.floor(h*0.068), 22, 44),
        langH      = math.clamp(math.floor(h*0.058), 18, 36),
        padding    = math.clamp(math.floor(h*0.012), 3,  10),
        minBtnW    = math.clamp(math.floor(w*0.138), 28, 44),
        minBtnH    = math.clamp(math.floor(h*0.052), 18, 32),
        navBtnW    = math.clamp(math.floor(w*0.138), 28, 44),
        navBtnH    = math.clamp(math.floor(h*0.052), 18, 32),
        cardH      = math.clamp(math.floor(h*0.13),  54, 80),
        grpCardH   = math.clamp(math.floor(h*0.115), 50, 72),
    }
end
...
local function BuildMainGUI(token)
    if token ~= _G_TOKEN then 
        player:Kick("HDLE Security: Unauthorized Execution Attempt.") 
        return 
    end
    
    if not IsEnvironmentSafe() then
        player:Kick("HDLE Security: Tampering Detected.")
        return
    end
    local DM = DESIGN.MainFrame
    local vp = workspace.CurrentCamera.ViewportSize
    local scale = math.clamp(math.min(vp.X/1920, vp.Y/1080), 0.5, 1.0)
    State.frameW = math.floor(DM.DefaultWidth  * scale)
    State.frameH = math.floor(DM.DefaultHeight * scale)
    ...
    ScreenGui.Name = "HDLE_MainGUI"
    ScreenGui.ResetOnSpawn = false ScreenGui.DisplayOrder = 10

    local EnablePlayerESP -- Forward declaration for first run combat trigger
    local Frame = Instance.new("Frame", ScreenGui)
    Frame.Size             = UDim2.new(0, State.frameW, 0, State.frameH)
    Frame.Position         = UDim2.new(DM.PosScaleX, 0, DM.PosScaleY, 0)
    Frame.BackgroundColor3 = DM.Background
    Frame.Active           = true Frame.Draggable = true Frame.ClipsDescendants = true
    MakeCorner(Frame, DM.CornerRadius) MakeStroke(Frame, Color3.fromRGB(0,255,150), 1)

    local PageContainer = Instance.new("Frame", Frame)
    PageContainer.Size=UDim2.new(1,0,1,0) PageContainer.BackgroundTransparency=1 PageContainer.ClipsDescendants=true

    -- ============================================================
    -- صفحة 1: الحركة
    -- ============================================================
    local MovePage = Instance.new("Frame", PageContainer)
    MovePage.Size=UDim2.new(1,0,1,0) MovePage.BackgroundTransparency=1 MovePage.Visible=true

    local MoveScroll = Instance.new("ScrollingFrame", MovePage)
    MoveScroll.Size=UDim2.new(1,0,1,0) MoveScroll.BackgroundTransparency=1
    MoveScroll.ScrollBarThickness=0 MoveScroll.ScrollingEnabled=false
    MoveScroll.CanvasSize=UDim2.new(0,0,0,0) MoveScroll.ClipsDescendants=true

    local MoveList = Instance.new("UIListLayout", MoveScroll)
    MoveList.HorizontalAlignment=Enum.HorizontalAlignment.Center MoveList.SortOrder=Enum.SortOrder.LayoutOrder

    local MovePad = Instance.new("UIPadding", MoveScroll)
    MovePad.PaddingLeft=UDim.new(0,6) MovePad.PaddingRight=UDim.new(0,6)
    MovePad.PaddingTop=UDim.new(0,4) MovePad.PaddingBottom=UDim.new(0,4)

    local function MakeBtn(parent, bgColor, order)
        local S = CalcSizes(State.frameW, State.frameH)
        local b = Instance.new("TextButton", parent)
        b.LayoutOrder=order b.Size=UDim2.new(1,0,0,S.btnH)
        b.BackgroundColor3=bgColor or Color3.fromRGB(60,60,60)
        b.TextColor3=Color3.new(1,1,1) b.Font=Enum.Font.SourceSansBold
        b.TextSize=S.fontNormal b.Text="" b.TextWrapped=false
        MakeCorner(b,8) return b
    end
    local function MakeInput(parent, def, order)
        local S = CalcSizes(State.frameW, State.frameH)
        local inp = Instance.new("TextBox", parent)
        inp.LayoutOrder=order inp.Size=UDim2.new(1,0,0,S.inputH)
        inp.BackgroundColor3=Color3.fromRGB(10,10,10)
        inp.TextColor3=Color3.fromRGB(0,255,100) inp.Text=def or ""
        inp.Font=Enum.Font.SourceSans inp.TextSize=S.fontNormal
    ...
    -- شريط العنوان (الحركة)
    local TitleBar = Instance.new("Frame", MoveScroll)
    TitleBar.LayoutOrder=1 TitleBar.Size=UDim2.new(1,0,0,S0.titleH) TitleBar.BackgroundTransparency=1

    local TitleLabel = Instance.new("TextLabel", TitleBar)
    TitleLabel.Size=UDim2.new(1,-(S0.minBtnW + (S0.navBtnW*3) + 20),1,0) TitleLabel.Position=UDim2.new(0,6,0,0)
    TitleLabel.BackgroundTransparency=1 TitleLabel.TextColor3=Color3.new(1,1,1)
    TitleLabel.TextSize=S0.fontTitle TitleLabel.Font=Enum.Font.SourceSansBold
    TitleLabel.TextXAlignment=Enum.TextXAlignment.Left TitleLabel.TextWrapped=false TitleLabel.Text=T("title")

    local MinBtn = Instance.new("TextButton", TitleBar)
    MinBtn.Size=UDim2.new(0,S0.minBtnW,0,S0.minBtnH)
    MinBtn.Position=UDim2.new(1,-(S0.minBtnW+4),0.5,-S0.minBtnH/2)
    MinBtn.BackgroundColor3=DESIGN.MinimizeBtn.Background MinBtn.TextColor3=DESIGN.MinimizeBtn.TextColor
    MinBtn.Font=Enum.Font.SourceSansBold MinBtn.TextSize=S0.fontNormal
    MinBtn.Text=DESIGN.MinimizeBtn.Text MinBtn.TextWrapped=false
    MakeCorner(MinBtn,DESIGN.MinimizeBtn.CornerRadius)

    -- زر 📌 للانتقال لصفحة المجموعات
    local NavLocBtn = Instance.new("TextButton", TitleBar)
    NavLocBtn.Size=UDim2.new(0,S0.navBtnW,0,S0.navBtnH)
    NavLocBtn.Position=UDim2.new(1,-(S0.minBtnW+S0.navBtnW+8),0.5,-S0.navBtnH/2)
    NavLocBtn.BackgroundColor3=Color3.fromRGB(0,100,180)
    NavLocBtn.TextColor3=Color3.new(1,1,1) NavLocBtn.Font=Enum.Font.SourceSansBold
    NavLocBtn.TextSize=S0.fontNormal NavLocBtn.Text="📌" NavLocBtn.TextWrapped=false
    MakeCorner(NavLocBtn,DESIGN.MinimizeBtn.CornerRadius)

    -- زر 👁 للوصول للتتبع
    local NavTrackerBtn = Instance.new("TextButton", TitleBar)
    NavTrackerBtn.Size=UDim2.new(0,S0.navBtnW,0,S0.navBtnH)
    NavTrackerBtn.Position=UDim2.new(1,-(S0.minBtnW+S0.navBtnW*2+12),0.5,-S0.navBtnH/2)
    NavTrackerBtn.BackgroundColor3=Color3.fromRGB(160,35,90)
    NavTrackerBtn.TextColor3=Color3.new(1,1,1) NavTrackerBtn.Font=Enum.Font.SourceSansBold
    NavTrackerBtn.TextSize=S0.fontNormal NavTrackerBtn.Text="👁" NavTrackerBtn.TextWrapped=false
    MakeCorner(NavTrackerBtn,DESIGN.MinimizeBtn.CornerRadius)

    -- زر 🎯 الموديول القتالي
    local NavCombatBtn = Instance.new("TextButton", TitleBar)
    NavCombatBtn.Size=UDim2.new(0,S0.navBtnW,0,S0.navBtnH)
    NavCombatBtn.Position=UDim2.new(1,-(S0.minBtnW+S0.navBtnW*3+16),0.5,-S0.navBtnH/2)
    NavCombatBtn.BackgroundColor3=Color3.fromRGB(200,80,0)
    NavCombatBtn.TextColor3=Color3.new(1,1,1) NavCombatBtn.Font=Enum.Font.SourceSansBold
    NavCombatBtn.TextSize=S0.fontNormal NavCombatBtn.Text="🎯" NavCombatBtn.TextWrapped=false
    MakeCorner(NavCombatBtn,DESIGN.MinimizeBtn.CornerRadius)

    local DoMinimize = BuildMinimizeSystem(Frame, ScreenGui)
    MinBtn.MouseButton1Click:Connect(DoMinimize)

    -- صف Up/Fwd/SharedInp
    local Row = Instance.new("Frame", MoveScroll)
    Row.LayoutOrder=2 Row.Size=UDim2.new(1,0,0,S0.rowH) Row.BackgroundTransparency=1

    local BtnUp = Instance.new("TextButton", Row)
    BtnUp.Size=UDim2.new(0.34,-3,1,0) BtnUp.Position=UDim2.new(0,0,0,0)
    BtnUp.BackgroundColor3=Color3.fromRGB(60,60,60) BtnUp.TextColor3=Color3.new(1,1,1)
    BtnUp.Font=Enum.Font.SourceSansBold BtnUp.TextSize=S0.fontSmall BtnUp.Text=T("up") BtnUp.TextWrapped=false
    MakeCorner(BtnUp,8)

    local BtnFwd = Instance.new("TextButton", Row)
    BtnFwd.Size=UDim2.new(0.34,-3,1,0) BtnFwd.Position=UDim2.new(0.34,3,0,0)
    BtnFwd.BackgroundColor3=Color3.fromRGB(60,60,60) BtnFwd.TextColor3=Color3.new(1,1,1)
    BtnFwd.Font=Enum.Font.SourceSansBold BtnFwd.TextSize=S0.fontSmall BtnFwd.Text=T("fwd") BtnFwd.TextWrapped=false
    MakeCorner(BtnFwd,8)

    local SharedInp = Instance.new("TextBox", Row)
    SharedInp.Size=UDim2.new(0.32,-3,1,0) SharedInp.Position=UDim2.new(0.68,3,0,0)
    SharedInp.BackgroundColor3=Color3.fromRGB(15,15,15) SharedInp.TextColor3=Color3.fromRGB(255,100,100)
    SharedInp.Text="10" SharedInp.Font=Enum.Font.SourceSans SharedInp.TextSize=S0.fontSmall
    SharedInp.ClearTextOnFocus=false SharedInp.TextWrapped=false
    MakeCorner(SharedInp,8)

    local LangBtn    = MakeBtn(MoveScroll, Color3.fromRGB(45,45,90), 4)
    ...
    CreateCToggle("aim_enabled", State.Combat.Aimbot, "Enabled", CmbScroll, function(val)
        if val then
            ...
            if readfile and writefile then
                if isfile and isfile("HDLE_Combat_Played.txt") then
                    isFirstTime = false
                elseif not isfile then
                    local ok, res = pcall(readfile, "HDLE_Combat_Played.txt")
                    if ok and res == "played" then isFirstTime = false end
                end
            end
            
            if isFirstTime then
                if writefile then pcall(writefile, "HDLE_Combat_Played.txt", "played") end
                State.Combat.Aimbot.AutoShoot = true
                State.Combat.ESP.Enabled = true
                State.Combat.ESP.Boxes = true
                State.Combat.ESP.Names = true
                UpdateAllCombatUI()
                
                if not State.showPlayerESP and EnablePlayerESP then
                    EnablePlayerESP()
                    -- Update standard ESP button color to show it was enabled automatically
                    pcall(function() BtnESP.BackgroundColor3 = Color3.fromRGB(0, 150, 70) end)
                end
            end
        end
    end)
...
RunService.RenderStepped:Connect(function()
    ...
    if State.Combat.Aimbot.Enabled then
        ...
        if State.Combat.Aimbot.AutoShoot and target then
            local shootKey = State.Combat.Aimbot.ShootKey
            local tool = player.Character and player.Character:FindFirstChildOfClass("Tool")
            if tool then tool:Activate() end
            SimulateClick(shootKey)
        end
